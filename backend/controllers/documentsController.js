...
exports.getDocumentById = withErrorHandling(async (req, res) => {
    const pool = await getConnection();
    ...
    const document = getRecordOrNull(result);
    if (!document) {
        return sendNotFound(res, 'Tài liệu không tồn tại');
    }
    res.json(document);
});

// POST /api/documents - Tạo tài liệu mới
exports.createDocument = withErrorHandling(async (req, res) => {
    ...
    if (!title || !file_name || !file_path) {
        return sendBadRequest(res, 'Title, file_name và file_path là bắt buộc');
    }
...
exports.updateDocument = withErrorHandling(async (req, res) => {
...
exports.deleteDocument = withErrorHandling(async (req, res) => {
    const pool = await getConnection();
    const result = await pool.request()
        .input('id', sql.Int, req.params.id)
